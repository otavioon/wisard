...
def generate_dietzfelbinger_values(num_inputs, num_entries, num_hashes):
    ...
    a_values = 2 * np.random.randint(
        0, 1 <<
        (num_inputs - 1), num_hashes) + 1  # Odd integers < 2**num_inputs
    b_values = np.random.randint(
        0, 1 << (num_inputs - int(np.log2(num_entries))),
        num_hashes)  # Odd integers < 2**num_inputs-log(num_entries)
...
def h3_hash(xv, m):
    ...
    reduction_result = np.zeros(
        m.shape[0], dtype=np.int64)  # ".reduce" is unsupported in Numba
...
class BloomFilter(Filter):
    ...
    def __init__(self, num_inputs, num_entries, num_hashes, random_values=None):
        ...
        if random_values is None:
            ...
            self.hash_values = generate_h3_values(num_inputs, num_entries,
                                                  num_hashes)
        else:
    ...
    def __check_membership(xv, hash_values, bleach, data, soft_error_rate):
        ...
        if soft_error_rate > 0.0:
            ...
            soft_error_vector = np.random.binomial(1, soft_error_rate,
                                                   hash_results.size)
            hash_results ^= soft_error_vector
        least_entry = data[hash_results].min(
        )  # The most times the entry has possibly been previously seen
    ...
    def check_membership(self, xv, soft_error_rate=0.0):
        return BloomFilter.__check_membership(xv, self.hash_values, self.bleach,
                                              self.data, soft_error_rate)
    ...
    def __add_member(xv, hash_values, data):
        ...
        data[hash_results] = np.maximum(data[hash_results], least_entry +
                                        1)  # Increment upper bound
    ...
    def corrupt(self, persistent_error_rate):
        if (not hasattr(self, "persistent_error_vector")) or (
                self.persistent_error_vector is None):
            self.persistent_error_vector = np.zeros(self.num_entries, dtype=int)
        else:
            assert (0)
        error_vector = np.random.binomial(1, persistent_error_rate,
                                          self.num_entries)
