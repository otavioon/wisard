...
class PermuteSampleValues:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray = None):
        if X.ndim > 1:
            ...
            x = X
        self._permutation = np.random.permutation(x.size)
        return self
    ...
    def transform(self, X: np.ndarray):
        ...
        return np.array([x[self._permutation] for x in samples])
...
class Discretizer:
    ...
    def transform(self, X):
        ...
        for sample in tqdm.tqdm(X):
            new_sample = []
            for x in sample.reshape(-1, 8):
                res = 0
                for i, v in enumerate(reversed(x)):
                    res += int((1 << i) * int(v))
                # res = [
                #     int(i) for i in "".join(["1"] * res).rjust(2**self.resolution, "0")
                # ],
                new_sample.append(res)
            samples.append(np.array(new_sample, dtype=np.uint8).ravel())
...
def distributive_thermometer_encoder(
    ...
    logging.info("Encoding dataset...")
    distributive_thermometer = DistributiveThermometerEncoder(resolution=resolution)
    if fit_on_train_test:
        x_merged = np.concat([x_train, x_test])
    else:
        x_merged = x_train
    distributive_thermometer.fit(x_merged)
    x_train = encode_dataset(distributive_thermometer, x_train, use_tqdm=use_tqdm)
...
def do_train_and_evaluate(
    ...
    if isinstance(bleach, int):
        y_pred = model.predict(
            x_test, y_test, bleach=bleach, min_bleach=min_bleach, use_tqdm=use_tqdm
        )
        ...
        return model, y_pred, ties, accuracy, history
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dataset_path", type=str, action="store", help="Input dataset file (pickle)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Discretization size",
    )

    parser.add_argument(
        "--tuple-size", type=int, help="Size of the tuple for Wisard", default=1
    )
    ...
    parser.add_argument(
        "--save-model", type=str, action="store", help="Path to store model"
    )
    parser.add_argument(
        "--linear-index", action="store_true", help="Use linear index for RAMs"
    )
    parser.add_argument("--verbose", action="store_true", help="Increases verbosity")
    parser.add_argument(
        "--bleach",
        action="store",
        default="bin_search",
    ...
    subparsers = parser.add_subparsers(
        help="Encoder parser", required=True, dest="encoder"
    )

    # The thermometer
    tparser = subparsers.add_parser("thermometer", help="Thermometer encoding")
    tparser.add_argument(
        "--resolution", type=int, required=True, help="Thermometer resolution"
    )

    # The distribuive thermometer
    dtparser = subparsers.add_parser(
        "distributive-thermometer", help="Distributive thermometer encoding"
    )
    dtparser.add_argument(
        "--resolution", type=int, required=True, help="Thermometer resolution"
    )
    dtparser.add_argument(
        "--fit-on-train-test", action="store_true", help="Concat train and test to fit"
    )
    ...
    # Permute and discretize for training
    permuter = PermuteSampleValues()
    permuter.fit(x_train)
    x_train = permuter.transform(x_train)
    x_test = permuter.transform(x_test)
    x_train = Discretizer(resolution=args.discretization).fit_transform(x_train)
    x_test = Discretizer(resolution=args.discretization).fit_transform(x_test)
    
    # Tuple indexes
    input_indexes = (
        np.arange(x_train[0].size)
    )
    # Use linear or shuffle indexes?
